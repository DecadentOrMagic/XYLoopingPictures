...
#define XY_CFWidth  self.frame.size.width
#define XY_CFHeight  self.frame.size.height
#define XY_CFY self.frame.origin.y
#import "XY_CarouselFigure.h"

@interface XY_CarouselFigure ()<UIScrollViewDelegate>
...
@implementation XY_CarouselFigure
...
- (void)frameAction{
    [self layoutIfNeeded];
    self.contentOffset = CGPointMake(XY_CFWidth, 0);
    self.contentSize = CGSizeMake(XY_CFWidth * 3, XY_CFHeight);
    //初始化3个imageView
    _leftImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, XY_CFWidth, XY_CFHeight)];
    [self addSubview:_leftImageView];
    _centerImageView = [[UIImageView alloc]initWithFrame:CGRectMake(XY_CFWidth, 0, XY_CFWidth, XY_CFHeight)];
    [self addSubview:_centerImageView];
    _rightImageView = [[UIImageView alloc]initWithFrame:CGRectMake(XY_CFWidth*2, 0, XY_CFWidth, XY_CFHeight)];
...
//时间方法
- (void)andTimeAction{
    [self setContentOffset:CGPointMake(XY_CFWidth * 2, 0) animated:YES];
    _isTimeUp = YES;
...
- (void)setSd_URLArray:(NSArray *)sd_URLArray{
    ...
    _pageControl.frame = CGRectMake(XY_CFWidth - 20 * _pageControl.numberOfPages, XY_CFHeight + XY_CFY - 20 + height, 20 * _pageControl.numberOfPages, 20);
    ...
    if (array.count == 1) {
        ...
        self.contentSize = CGSizeMake(XY_CFWidth, XY_CFHeight);
        [_timer invalidate];
        _timer = nil;
        return;
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    //向左滑动
    else if(self.contentOffset.x == XY_CFWidth * 2)
    {
        _imageArrayIndex++;
        //判断计算
...
- (void)addTouchImageActionBlock:(void (^)(NSInteger ))block{
    __block XY_CarouselFigure *men = self;
    self.block = ^(){
        if (block) {
            //多张情况
